...
public class BTreeImpl<K extends Comparable<K>, V> implements BTree<K, V> {
    ...
    private Entry get(Node currentNode, K key, int height) {
        Entry[] entries = currentNode.entries;

        //current node is external (i.e. height == 0)
        if (height == 0) {
            for (int j = 0; j < currentNode.entryCount; j++) {
                if(isEqual(key, (K) entries[j].key)) {
        ...
        //current node is internal (height > 0)
            ...
            for (int j = 0; j < currentNode.entryCount; j++) {
                ...
                if (j + 1 == currentNode.entryCount || less(key, (K) entries[j + 1].key)) {
    ...
    public V put(K k, V v) {
        ...
        if(alreadyThere != null) {
            if(alreadyThere.val instanceof URI){
                try {
                    alreadyThere.val = this.persistenceManager.deserialize(k);
                    this.persistenceManager.delete(k);
                } catch (IOException e) {
                    return null;
                }
            }
            V previous = (V)alreadyThere.val;
            alreadyThere.val = v;
            return previous;
        }
        ...
        this.height++;
        return null; //dont know what to return
    }
    ...
    private Node put(Node currentNode, K key, V val, int height) {
        ...
        if (height == 0) {
            ...
            for (j = 0; j < currentNode.entryCount; j++) {
                if (less(key, (K) currentNode.entries[j].key)) {
                    break;
                }
            }
        }
        ...
        // internal node
            ...
            for (j = 0; j < currentNode.entryCount; j++) {
                ...
                if ((j + 1 == currentNode.entryCount) || less(key, (K) currentNode.entries[j + 1].key)) {
    ...
    @Override
    public void moveToDisk(K k) throws Exception {
        Entry entry = get(this.root, k, this.height);
        if(entry.val == null) throw new NoSuchElementException();
        if(!(entry.val instanceof URI)) {
            URI location = URI.create(k.toString());
            this.persistenceManager.serialize(k, (V) entry.val);
            entry.val = location;
        }
    }

    @Override
    public void setPersistenceManager(PersistenceManager<K, V> pm) {
